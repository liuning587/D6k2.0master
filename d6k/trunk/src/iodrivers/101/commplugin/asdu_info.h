...
enum
{
	DEVICE_STATE_UNKNOWN,//初始状态
	DEVICE_STATE_LINKRESET,//已复位远方链路
	DEVICE_STATE_LINKSTATE,//已请求链路状态

	CONTROL_STATE_LINKSTATE, //装置侧请求链路状态
	CONTROL_STATE_LINKRESET, //装置侧请求复位状态

	DEVICE_STATE_GENERALCALL,//启动过程总召
	
	DEVICE_STATE_GENERALCALL_OVER,//启动过程总召
	
	DEVICE_STATE_SYNCTIME,//启动过程对时  

	DEVICE_STATE_SYNCTIME_RDTIME,//同步对时结束，启动读取时间

	DEVICE_STATE_HEART, //心跳测试报文

	DEVICE_STATE_NONE,//普通确认帧
};

enum
{
	DEVICE_RUN_STATE_UNKNOWN,//初始态
	DEVICE_RUN_STATE_SEND,//已发送
	DEVICE_RUN_STATE_RECV,//已接收
};

enum
{
	MAX_ERRORS_TO_HALT = 10,    //ERROR达到该值，则挂起该装置
	MAX_HALT_TIME = 5, //装置挂起最大时间(分钟)
};
...
enum
{
	...
	D_FIX_SEARCH = 201,   //获取a定值区号
	D_FIX_READ = 202,     //定值读取
	D_FIX_WRITE = 203,    //定值写入

	D_FILE_TRANSPORT = 210,   //文件服务
	D_UPDATE_PROCESS = 211,   //升级
...
{
public:
	unsigned char m_linkaddr0;
	unsigned char m_linkaddr1;
public:
	void SetCOI(unsigned short cot)
	{
		m_linkaddr0 = MYLO8(cot);
		m_linkaddr1 = MYHI8(cot);
	}
	unsigned short GetCOI()
	{
		return MYMAKE16(m_linkaddr0, m_linkaddr1);
	}
};
...
public:
	...
	unsigned char m_linkaddr1;
public:
	void SetAddr(unsigned short cot)
	{
		m_linkaddr0 = MYLO8(cot);
		m_linkaddr1 = MYHI8(cot);
	}
...
public:
	unsigned char m_linkaddr0;
	unsigned char m_linkaddr1;
	unsigned char m_linkaddr2;
public:
	void SetCot(unsigned short addr)
	{
		unsigned short  lo16 = MYLO16(addr);
		unsigned short  hi16 = MYHI16(addr);

		m_linkaddr0 = MYLO8(lo16);
		m_linkaddr1 = MYHI8(lo16);
		m_linkaddr2 = MYLO8(hi16);
	}
	unsigned short GetCot()
	{
		unsigned short lo16 = MYMAKE16(m_linkaddr0, m_linkaddr1);
		unsigned short hi16 = MYMAKE16(m_linkaddr2, 0);
...
public:
	void SetAddr(int addr)
	{
		...
		m_linkaddr2 = MYLO8(hi16);
		m_linkaddr3 = MYHI8(hi16);
	}

	int GetAddr()
	{
		unsigned short lo16 = MYMAKE16(m_linkaddr0, m_linkaddr1);
		unsigned short hi16 = MYMAKE16(m_linkaddr2, m_linkaddr3);
...
//升级
class ASDU211_UPDATE : public ASDU_BASE
{
public:
	INFOADDR      infoaddr;
	QDS  m_qds;
...
//测试命令
class ASDU104 : public ASDU_BASE
{
public:
	INFOADDR m_infoaddr;
	//CP56Time2a m_time;
...
public:
	INFOADDR m_infoaddr;
	ASDUADDR m_infoFileIndex;    //文件名称编号
	INFOADDR m_FileLeg;          //文件长度
...
public:
	...
	ASDUADDR m_infoCurrentAreaIndex;     //当前定值区区号
	ASDUADDR m_infoMinAreaIndex;         //最小定值区区号

	ASDUADDR m_infoMaxAreaIndex;         //最大定值区区号
};
