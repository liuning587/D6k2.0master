...
CFixdDeploy::CFixdDeploy(CCommThread *pCommunicate, IMainModule *pCore, const QString &deviceName, QWidget *parent)
	...
	m_pLineEdDvArea(new QLineEdit(this)),
	m_strDeviceName(deviceName)
{

	m_pLineEdDvArea->setFixedWidth(280);
	m_pLineEdDvArea->setPlaceholderText(tr("定值区号"));
...
void CFixdDeploy::Slot_SendUpdatePoints()
{
	...
	pWgt->deleteLater();
	*/
	GetFtuModel()->SetFixCode(m_pLineEdDvArea->text().toInt());
...
void CFixdDeploy::Slot_SendCurrentPoints()
{
	...
	int messReturn = QMessageBox::information(this, tr("提示"), tr("确定修改定值吗 ?"), QMessageBox::Yes, QMessageBox::No);
...
void CFixdDeploy::resizeEvent(QResizeEvent * event)
{
	m_pLineEdDvArea->move(this->width() - 300, 10);
