...
void CLoadFileThread::Slot_CreateNewFile(QString strFileName, QByteArray byData)
{
	...
	if (strFileName.contains("/prog/prot_list.ini"))
	{
		//点表文件
		strFileName = strRunPath + REMOTE_TABLE + GetFtuModel()->GetConfigwgt()->GetPointTable() + "_" + strFileName.remove("//prog/");
