...
void CDevIecConfigWgt::Slot_ContextMenuRequest(const QPoint & pointCurrent)
{
	...
	QAction *pUpdateAllAct = new QAction(tr("更新所有"), pCurrentTable);
	QAction *pSendAllAct = new QAction(tr("发送所有"), pCurrentTable);
	//保存
...
void CDevIecConfigWgt::Slot_GetAllPoints()
{
	...
	if (pClickAct->parent() != nullptr)
	{
		...
		if (pCurrentTableWgt != NULL)
		{
			...
			m_pCore->LogString(m_strDeviceName.toLocal8Bit().data(), tr("发送规约配置读取请求").toLocal8Bit().data(), 1);
		}
	}
}
...
void CDevIecConfigWgt::Slot_SendCurrentAllPoints()
{
	...
	int messReturn = QMessageBox::information(this, tr("提示"), tr("是否确定修改?"), QMessageBox::Yes, QMessageBox::No);
	if (messReturn == QMessageBox::No)
	{
		return;
	}
	...
	if (pClickAct->parent() != nullptr)
	{
		...
		if (pCurrentTableWgt != NULL)
		{
			...
			//启动定时器
			//m_pFixReadTimeOut->start(5000);
			m_pCore->LogString(m_strDeviceName.toLocal8Bit().data(), tr("发送规约配置写入请求").toLocal8Bit().data(), 1);
		}
	}
}
