...
void CRealTimeAnalog::InitWidget()
{
	...
	lstHorizonLables << tr("Remote NUM") << tr("Device NUM") << tr("Destriber") << tr("Value") << tr("Unit") << tr("Binary Type") << tr("Time")<<"ZoomHidden"<<tr("最大值")<<tr("最小值");
	m_pTableModel->setHorizontalHeaderLabels(lstHorizonLables);
    
	m_pBtnRefresh = new QPushButton(this);
	m_pBtnRefresh->setText("极值刷新");
	m_pBtnRefresh->setFixedWidth(150);

	m_pLineEdTime = new QLineEdit(this);
	m_pLineEdTime->setReadOnly(true);
	m_pLineEdTime->setFixedWidth(300);

	QHBoxLayout *pMaxMinLayout = new QHBoxLayout;
	pMaxMinLayout->addStretch();
	pMaxMinLayout->addWidget(m_pBtnRefresh);
	pMaxMinLayout->addWidget(m_pLineEdTime);

	QVBoxLayout *pWgtlayout = new QVBoxLayout(this);

	pWgtlayout->addLayout(pMaxMinLayout);
	pWgtlayout->addWidget(m_pTableView);

    m_pTableView->setColumnHidden(7, true);
	m_pTableView->setColumnHidden(5, true);
	...
	//
	connect(m_pBtnRefresh,SIGNAL(clicked()),this,SLOT(Slot_RefreshMaxMinData()));
}
...
//         }
		...
		if (m_pTableModel->item(iUpdateRow, 8)->text().isEmpty())
		{
			m_pTableModel->item(iUpdateRow, 8)->setText(QString::number(nValue, 'f', 3));
		}
		else
		{
			if (m_pTableModel->item(iUpdateRow, 8)->text().toFloat() < nValue)
			{
				m_pTableModel->item(iUpdateRow, 8)->setText(QString::number(nValue, 'f', 3));
			}
		}

		if (m_pTableModel->item(iUpdateRow, 9)->text().isEmpty())
		{
			m_pTableModel->item(iUpdateRow, 9)->setText(QString::number(nValue, 'f', 3));
		}
		else
		{
			if (m_pTableModel->item(iUpdateRow, 9)->text().toFloat() > nValue)
			{
				m_pTableModel->item(iUpdateRow, 9)->setText(QString::number(nValue, 'f', 3));
			}
		}
...
//         }
		...
		//
		m_pTableModel->setItem(iRowNum, 8, new QStandardItem(QString::number(nValue, 'f', 6)));
		//
		m_pTableModel->setItem(iRowNum, 9, new QStandardItem(QString::number(nValue, 'f', 6)));
...
void CRealTimeAnalog::Slot_ContextMenuRequest(const QPoint &pPoint)
{
	...
	//隐藏
	QAction *pHidenAct = new QAction(tr("复位"), m_pTableView);


    pMenu->addAction(pZoneAct);
	pMenu->addAction(pHidenAct);

    connect(pZoneAct, SIGNAL(triggered()), this, SLOT(Slot_DeadZoneAct()));
	connect(pHidenAct, SIGNAL(triggered()), this, SLOT(Slot_HiddenMData()));
...
void CRealTimeAnalog::Slot_HiddenMData()
{
	if (m_bHiddenFlag)
	{
		m_pTableView->setColumnHidden(8, m_bHiddenFlag);
		m_pTableView->setColumnHidden(9, m_bHiddenFlag);
		m_bHiddenFlag = !m_bHiddenFlag;
	}
	else
	{
		m_pTableView->setColumnHidden(8, m_bHiddenFlag);
...
void CRealTimeAnalog::Slot_RefreshMaxMinData()
{
	for (int i=0; i<m_pTableModel->rowCount(); i++)
	{
		m_pTableModel->item(i, 8)->setText("");
		m_pTableModel->item(i, 9)->setText("");
	}
	m_pLineEdTime->setText(QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss zzz"));
}
...
void CRealTimeAnalog::InitData()
{
	...
	while (i != mapPoint.constEnd())
	{
		...
		//最小值
		m_pTableModel->setItem(iRowNum, 9, new QStandardItem(""));
		//记录数据
