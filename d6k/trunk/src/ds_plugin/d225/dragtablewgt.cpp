    ...
    for (int begin=0; begin<selectedRanges().count(); begin++)
    {
        ...
        for (int row = selection.topRow(), firstRow = row;row <= selection.bottomRow(); row++)
        {
            for (int col = selection.leftColumn(); col <= selection.rightColumn(); col++)
            {
				...
				if (col == selection.leftColumn())
				{
					selectionString.append(index.data().toString()+ "|" +index.data(Qt::UserRole+1).toString()).append("\t");
				}
				else
				{
...
//             }
		...
		else
		{
			...
			QString strDeviceNumbs = row.at(0).trimmed().replace('"', "");


			QTableWidgetItem *pItem0 = new QTableWidgetItem;
			pItem0->setText(QString::number(rowCount));
			pItem0->setData(Qt::UserRole + 1, strDeviceNumbs.split("|").last());

			setItem(rowCount, 0, pItem0);
			setItem(rowCount, 1, new QTableWidgetItem(strDeviceNumbs.split("|").first()));
...
void CDragTableWgt::Slot_setRecoverState(QList<unsigned int> lstIDs)
{
	...
	{
		if (lstIDs.contains(item(i, 1)->text().toUInt()) && item(i, 1)->isSelected())
		{
			this->removeRow(i);
		}
	}
