...
CFtuModule::CFtuModule()
{
	...
	if (qtPlugini->load(strRunPath + "/locales/d225-zh-CN.qm"))
	{
...
// 	drag2->exec();
	...
	m_pChoosePointWgt = new CChoosePointsWgt(m_pPointInfo, m_pConfigWgt->GetProjectName());
	...
	//串口
	m_pSerPortClientWgt = new CBinClientWgt;
    //录波
...
CFtuModule::~CFtuModule()
{
	...
	m_pSerPortClientWgt->deleteLater();
...
// 	pFtuItem->appendRow(pWeb);
	...
	QStandardItem *pSerport = new QStandardItem(tr("串口文件"));
	pSerport->setData(TREE_SERPOT_ITEM + m_iCurrentPluginIndex, Qt::UserRole);
	//pHistory->setIcon(QIcon(":/icons/analog.png"));
	pFtuItem->appendRow(pSerport);
...
void CFtuModule::Slot_ClickLeftTreeItem(const QModelIndex &qIndex)
{
	...
	else if (nType == TREE_SERPOT_ITEM + m_iCurrentPluginIndex)
	{
		//串口
		if (m_MIndexTab.contains(TREE_SERPOT_ITEM + m_iCurrentPluginIndex))
		{
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_SERPOT_ITEM + m_iCurrentPluginIndex]);
		}
		else
		{
			m_pMainWindow->GetTabWidget()->AddTab(m_pSerPortClientWgt, tr("串口文件"), tr("SerprotFile"));
			m_MIndexTab.insert(TREE_SERPOT_ITEM + m_iCurrentPluginIndex, m_pSerPortClientWgt);
			m_MTabIndex.insert(m_pSerPortClientWgt, TREE_SERPOT_ITEM + m_iCurrentPluginIndex);
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_SERPOT_ITEM + m_iCurrentPluginIndex]);
		}
	}
