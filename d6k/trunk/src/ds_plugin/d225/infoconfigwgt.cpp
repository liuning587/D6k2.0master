	...
	: QDialog(parent)
{
	...
	//根目录
	m_pRootDir = new QLineEdit(this);
	m_pRootDir->setText("/tffs0");
...
void  CInfoConfigWgt::InitWgt()
{
	...
	QLabel *prootDir = new QLabel(QStringLiteral("根目录:    "));
	...
	pRootLayout->addWidget(prootDir);
	pRootLayout->addWidget(m_pRootDir);
	pRootLayout->addStretch();
	m_pRootDir->setFixedWidth(240);
	...
	pPage1Layout->addLayout(pRootLayout);
	...
	pTabWgt->addTab(pTimeWgt,QStringLiteral("104规约时间"));
...
void CInfoConfigWgt::AnalyseXmlFile(QIODevice *pDevice)
{
    ...
    while (!xmlReader.atEnd())
    {
        ...
        if (xmlReader.isStartElement())
        {
			...
			else if (xmlReader.name().toString() == "CheckTimeFlag")
			{
				m_pComBoxCheckTime->setCurrentIndex(xmlReader.readElementText().toInt());
			}
...
void CInfoConfigWgt::WriteBaseInfo(QXmlStreamWriter &xmlWriter)
{
	...
	xmlWriter.writeStartElement("CheckTimeFlag");
	xmlWriter.writeCharacters(QString::number(m_pComBoxCheckTime->currentIndex()));
	xmlWriter.writeEndElement();
