...
void CRealTimeSOE::AnalyseRealSeoBinaryData(int iDeviceID, int nPointID, int nValue, int nQuality, QString pDateTime)
{
	...
	//系统时间
	if (pDateTime.split(" ").last().length() == 1)
	{
		pDateTime.insert(pDateTime.length() - 1, "00");
	}
	else if (pDateTime.split(" ").last().length() == 2)
	{
		pDateTime.insert(pDateTime.length() - 2, "0");
	}
	else if (pDateTime.split(" ").last().length() == 0)
	{
		pDateTime.insert(pDateTime.length(), "000");
	}
...
void CRealTimeSOE::Slot_ContextMenuRequest(const QPoint &point)
{
	...
	QAction *pRect = new QAction(tr("复位"),m_pTableView);
	pMenu->addAction(pRect);
	connect(pRect, SIGNAL(triggered()), this, SLOT(Slot_ResetAction()));
...
void CRealTimeSOE::Slot_ResetAction()
{
	if (m_bHiddenFlag)
	{
		m_pTableView->setColumnHidden(7,m_bHiddenFlag);
