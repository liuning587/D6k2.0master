...
void CFileTransWgt::Slot_GetPointTable()
{
	...
	/*QString fileName = qApp->applicationDirPath() + "/ini/f220/remotetable/" + GetFtuModel()->GetConfigwgt()->GetPointTable() + "_prot_list.ini";
...
void CFileTransWgt::Slot_DownLoadPointTable()
{
	...
	if (!fileName.isEmpty())
	{
		...
		tWriteAction.m_strFileName ="/prog/prot_list.ini";    //文件名称
...
void CFileTransWgt::Slot_RefreshItems()
{
	...
	*/
	...
	if (!m_strCurrentPath.isEmpty() && ui.treeWidget->topLevelItemCount() == 0)
	{
...
void CFileTransWgt::Slot_ItemDoubleClicke(QTreeWidgetItem * item, int column)
{
	...
	if (item->data(0,Qt::UserRole+1) == 1 || item->data(0, Qt::UserRole + 2).toInt() == 1)
	{
		//如果是目录就更新
		//目录名称
		if (!m_strCurrentPath.isEmpty() && item->data(0,Qt::UserRole+2).toInt() == 1)
		{
		...
		else
		{
			...
			ui.lineEdit->setText(ui.lineEdit->text().isEmpty()? strCatalogName : ui.lineEdit->text() + "/" + strCatalogName);
