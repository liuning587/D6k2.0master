...
void CDragTableWgt::SetSourceTableWgt()
{
	setColumnCount(6);
	QStringList headers;
	headers << tr("Num") << tr("InnerAddr") << tr("Address") <<tr("Name")<< tr("Desc") <<(tr("Unit"));
...
void CDragTableWgt::SetDescTableWgt()
{
	...
	headers2 << tr("RemoteNum") << tr("Num") << tr("InnerAddr") << tr("Address") <<tr("Name")<< tr("Desc") << (tr("Unit"));
...
QMap<unsigned int, RPT> CDragTableWgt::GetCurrentTableData()
{
	...
	for (int i = 0; i < this->rowCount(); i++)
	{
		...
		srpt.OccNO = this->item(i, 0)->text().toUInt();
		srpt.InnerAddr = this->item(i, 1)->text();
		srpt.Address = this->item(i, 2)->text();
		srpt.Name = this->item(i, 3)->text();
		srpt.Desccription=this->item(i, 4)->text();

		mapPointInfo.insert(srpt.OccNO, srpt);
	}
	return mapPointInfo;
}
