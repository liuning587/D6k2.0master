...
CFtuModule::CFtuModule()
{
	...
	if (qtPlugini->load(strRunPath + "/locales/fci-zh-CN.qm"))
	{
...
void CFtuModule::Init(IMainModule *pMainModule)
{
	...
	m_pDevInfo = new CDeviceData;

	m_pDevInfo->m_nDeviceAddr = 1;
	...
	connect(m_pCommThread, SIGNAL(Signal_SocketError(QString)), this, SLOT(Slot_SocketError(QString)),Qt::QueuedConnection);
	...
	//加载点表数据
	m_pFcicfg = new CFciFile();

	QString strRunPath = qApp->applicationDirPath();

	if (!m_pFcicfg->ReadPrgFile(strRunPath + POINTTABLEPATH + strPointTableName + ".ini"))
	{
		return;
	}
	CFciFile::GetInstance().ReadPrgFile(strRunPath + POINTTABLEPATH + strPointTableName + ".ini");
	
	m_pPointInfo = new CPointInfo;
	m_pChoosePointWgt = new CChoosePointsWgt(m_pPointInfo);
	m_pChoosePointWgt->InitWgt(m_pFcicfg);
	...
	//遥信实时数据
    m_pRealTimeBinaryWgt = new CRealTimeBinary(0, m_pCommThread, m_pFcicfg, m_pConfigWgt);
	//遥测实时数据
    m_pRealTimeAnalog = new CRealTimeAnalog(0, m_pCommThread, m_pPointInfo, m_pFcicfg, m_pConfigWgt);
	//SOE数据
	...
	m_pSysParaWgt = new CSysParaWgt(m_pFcicfg, nullptr);

	m_pUartParaWgt = new CUartPara(m_pFcicfg,nullptr);

	m_pGprsParaWgt = new CGprsPara(m_pFcicfg, nullptr);

	m_pGpsParaWgt = new CGpsPara(m_pFcicfg, nullptr);

	m_pSmsParaWgt = new CSmsPara(m_pFcicfg, nullptr);

	m_pProtocolParaWgt = new CProtocolPara(m_pFcicfg,nullptr);
...
{
	QByteArray byDestr = tr("Open Com Success").toLocal8Bit();
...
void CFtuModule::CreateTreeItem()
{
	...
	QStandardItem *pFciItem = new QStandardItem(strDeviceName);
	m_pPluginTopItem = pFciItem;
	pFciItem->setData(TREE_FTU_ITEM, Qt::UserRole);
	//pFtuItem->setIcon(QIcon(":/icons/ftu.png"));
	pTopItem->appendRow(pFciItem);

	m_iCurrentPluginIndex = pTopItem->rowCount()*1000;

	//syspara
	QStandardItem *pSysPara = new QStandardItem(tr("SysPara"));
	pSysPara->setData(TREE_SYSPARA + m_iCurrentPluginIndex, Qt::UserRole);
	//pSelectPoint->setIcon(QIcon(":/icons/analog.png"));
	pFciItem->appendRow(pSysPara);

	//uart
	QStandardItem *pUart = new QStandardItem(tr("Uart"));
	pUart->setData(TREE_UART + m_iCurrentPluginIndex, Qt::UserRole);
	pFciItem->appendRow(pUart);

	//GPRS
	QStandardItem *pGprs = new QStandardItem(tr("GPRS"));
	pGprs->setData(TREE_GPRS + m_iCurrentPluginIndex, Qt::UserRole);
	pFciItem->appendRow(pGprs);

	//GPS
	QStandardItem *pGps = new QStandardItem(tr("GPS"));
	pGps->setData(TREE_GPS + m_iCurrentPluginIndex, Qt::UserRole);
	pFciItem->appendRow(pGps);


	QStandardItem *pSms = new QStandardItem(tr("SMS"));
	pSms->setData(TREE_SMS + m_iCurrentPluginIndex, Qt::UserRole);
	pFciItem->appendRow(pSms);

	QStandardItem *pProtocol = new QStandardItem(tr("Protocol"));
	pProtocol->setData(TREE_PROTOCOL + m_iCurrentPluginIndex, Qt::UserRole);
	pFciItem->appendRow(pProtocol);
	...
	pFciItem->appendRow(pSelectPoint);
	...
	pFciItem->appendRow(pRemoteSignal);
	...
	pFciItem->appendRow(pRemoteMeasure);
	...
	pFciItem->appendRow(pRemotControl);
	...
	pFciItem->appendRow(pRemotRemoteCot);
	//fix
	...
	pFciItem->appendRow(pFixValueSet);
	// devIec
	...
	pFciItem->appendRow(pIecValueSet);
...
void CFtuModule::Slot_ClickLeftTreeItem(const QModelIndex &qIndex)
{
	...
	else if (nType==TREE_SYSPARA+m_iCurrentPluginIndex)
	{
		if (m_MIndexTab.contains(TREE_SYSPARA + m_iCurrentPluginIndex))
		{
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_SYSPARA + m_iCurrentPluginIndex]);
		}
		else
		{
			m_pMainWindow->GetTabWidget()->AddTab(m_pSysParaWgt, tr("SysPara"), "SysPara");
			m_MIndexTab.insert(TREE_SYSPARA + m_iCurrentPluginIndex, m_pSysParaWgt);
			m_MTabIndex.insert(m_pSysParaWgt, TREE_SYSPARA + m_iCurrentPluginIndex);
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_SYSPARA + m_iCurrentPluginIndex]);
		}
	}
	else if (nType == TREE_UART + m_iCurrentPluginIndex)
	{
		if (m_MIndexTab.contains(TREE_UART + m_iCurrentPluginIndex))
		{
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_UART + m_iCurrentPluginIndex]);
		}
		else
		{
			m_pMainWindow->GetTabWidget()->AddTab(m_pUartParaWgt, tr("UART"), "UART");
			m_MIndexTab.insert(TREE_UART + m_iCurrentPluginIndex, m_pUartParaWgt);
			m_MTabIndex.insert(m_pUartParaWgt, TREE_UART + m_iCurrentPluginIndex);
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_UART + m_iCurrentPluginIndex]);
		}
	}
	else if (nType == TREE_GPRS + m_iCurrentPluginIndex)
	{
		if (m_MIndexTab.contains(TREE_GPRS + m_iCurrentPluginIndex))
		{
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_GPRS + m_iCurrentPluginIndex]);
		}
		else
		{
			m_pMainWindow->GetTabWidget()->AddTab(m_pGprsParaWgt, tr("GPRS"), "GPRS");
			m_MIndexTab.insert(TREE_GPRS + m_iCurrentPluginIndex, m_pGprsParaWgt);
			m_MTabIndex.insert(m_pGprsParaWgt, TREE_GPRS + m_iCurrentPluginIndex);
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_GPRS + m_iCurrentPluginIndex]);
		}
	}
	else if (nType == TREE_GPS + m_iCurrentPluginIndex)
	{
		if (m_MIndexTab.contains(TREE_GPS + m_iCurrentPluginIndex))
		{
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_GPS + m_iCurrentPluginIndex]);
		}
		else
		{
			m_pMainWindow->GetTabWidget()->AddTab(m_pGpsParaWgt, tr("GPS"), "GPS");
			m_MIndexTab.insert(TREE_GPS + m_iCurrentPluginIndex, m_pGpsParaWgt);
			m_MTabIndex.insert(m_pGpsParaWgt, TREE_GPS + m_iCurrentPluginIndex);
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_GPS + m_iCurrentPluginIndex]);
		}
	}
	else if (nType == TREE_SMS + m_iCurrentPluginIndex)
	{
		if (m_MIndexTab.contains(TREE_SMS + m_iCurrentPluginIndex))
		{
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_SMS + m_iCurrentPluginIndex]);
		}
		else
		{
			m_pMainWindow->GetTabWidget()->AddTab(m_pSmsParaWgt, tr("SMS"), "SMS");
			m_MIndexTab.insert(TREE_SMS + m_iCurrentPluginIndex, m_pSmsParaWgt);
			m_MTabIndex.insert(m_pSmsParaWgt, TREE_SMS + m_iCurrentPluginIndex);
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_SMS + m_iCurrentPluginIndex]);
		}
	}
	else if (nType == TREE_PROTOCOL + m_iCurrentPluginIndex)
	{
		if (m_MIndexTab.contains(TREE_PROTOCOL + m_iCurrentPluginIndex))
		{
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_PROTOCOL + m_iCurrentPluginIndex]);
		}
		else
		{
			m_pMainWindow->GetTabWidget()->AddTab(m_pProtocolParaWgt, tr("PROTOCOL"), "PROTOCOL");
			m_MIndexTab.insert(TREE_PROTOCOL + m_iCurrentPluginIndex, m_pProtocolParaWgt);
			m_MTabIndex.insert(m_pProtocolParaWgt, TREE_PROTOCOL + m_iCurrentPluginIndex);
			m_pMainWindow->GetTabWidget()->setCurrentWidget(m_MIndexTab[TREE_PROTOCOL + m_iCurrentPluginIndex]);
		}
	}
}
...
void CFtuModule::InitCallZt()
{
    ...
    //调用对时
    QAction *pSycsAct = new QAction(tr("SycsTime(T)"),pPlcMenu);
    pPlcMenu->addAction(pSycsAct);
	pPlcMenu->addSeparator();
    //调用电度召唤
    QAction *pRdTimeAct = new QAction(tr("ReadTime(T)"),pPlcMenu);
    pPlcMenu->addAction(pRdTimeAct);

	QAction *pTCAct = new QAction(tr("TestComand(C)"), pPlcMenu);
	pPlcMenu->addAction(pTCAct);
    ...
    connect(pRdTimeAct, SIGNAL(triggered()), this, SLOT(Slot_RdTimeAct()));
	connect(pTCAct, SIGNAL(triggered()), this, SLOT(slot_OnTestCommand()));
...
//调用电度
void CFtuModule::Slot_RdTimeAct()
{
    m_pCommThread->SetRdTime();
}
void CFtuModule::slot_OnTestCommand()
{
	m_pCommThread->SetTcCommand();
}
