...
void CRealTimeKwh::InitData()
{
	...
	while (i != mapPoint.constEnd())
	{
		...
		//插入数据
		//
		m_pTableModel->setItem(iRowNum, 0, new QStandardItem((i.value().Address)));
		//
		m_pTableModel->setItem(iRowNum, 1, new QStandardItem((i.value().InnerAddr)));
		//
		m_pTableModel->setItem(iRowNum, 2, new QStandardItem(i.value().Desccription));
		//点值
