...
void CAnalogDragTableWgt::SetSourceTableWgt()
{
	setColumnCount(9);
	QStringList headers;
	headers << tr("Num") << tr("InnerAddr") << tr("Address")<< tr("Name") << tr("Desc") 
		<< tr("MinValue") << tr("MaxValue") << tr("KRate") << tr("Unit");
...
void CAnalogDragTableWgt::SetDescTableWgt()
{
	...
	headers2 << tr("RemoteNum") << tr("Num") << tr("InnerAddr") << tr("Address") << tr("Desc") << tr("Name")
		<< tr("MinValue") << tr("MaxValue") << tr("KRate") << tr("Unit");
...
QMap<unsigned int, RPT> CAnalogDragTableWgt::GetCurrentTableData()
{
	...
	for (int i = 0; i < this->rowCount(); i++)
	{
		...
		srpt.Desccription = this->item(i, 3)->text();
