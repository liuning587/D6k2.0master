...
CRealtimeAbnormalWgt::CRealtimeAbnormalWgt(QWidget *parent)
	: QWidget(parent)
{
	ui.setupUi(this);
...
CRealtimeAbnormalWgt::~CRealtimeAbnormalWgt()
{
}

void CRealtimeAbnormalWgt::InitData()
{
	const std::map<int, std::shared_ptr<CSOFTSTRAP> > &pPointTable = GetBreakerModuleApi()->GetPointTable()->GetAbnormalInfo();
...
void CRealtimeAbnormalWgt::UpdateItem(int nValue)
{
	//转换为二进制字符串
