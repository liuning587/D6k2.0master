...
CRealtimeActionWgt::CRealtimeActionWgt(QWidget *parent)
	: QWidget(parent)
{
	ui.setupUi(this);
...
CRealtimeActionWgt::~CRealtimeActionWgt()
{
}


	void CRealtimeActionWgt::InitData()
	{
		const std::map<int, std::shared_ptr<CSOFTSTRAP> > &pPointTable = GetBreakerModuleApi()->GetPointTable()->GetActionInfo();
	...
	void CRealtimeActionWgt::UpdateItem(int nValue)
	{
		//转换为二进制字符串
