...
void CHisSave::DoubleClicked(const QModelIndex &index)
{
	...
	if (FES_TYPE_HIS_AI_REPORT_ITEM == nType)
	{
		//模拟量报表数据保存
		//预留接口
		CAIReportSaveView *pView = new CAIReportSaveView(nullptr);
	...
	else if (FES_TYPE_HIS_AI_CURVE_ITEM == nType)
	{
		//模拟量曲线数据保存
		//预留接口
		CAICurveSaveView *pView = new CAICurveSaveView(nullptr);
		m_pCore->GetUIMgr()->GetTabWidget()->addTab(pView, "test");
	}
	else if (FES_TYPE_HIS_AI_MAX_MIN_ITEM == nType)
	{
		//模拟量最值统计
		//预留接口
		CAIMaxMinStatisticsView *pView = new CAIMaxMinStatisticsView(nullptr);
		m_pCore->GetUIMgr()->GetTabWidget()->addTab(pView, "test");
	}
	else if (FES_TYPE_HIS_AI_LIMIT_ITEM == nType)
	{
		//模拟量越限值统计
		//预留接口
		CAILimitStatisticsView *pView = new CAILimitStatisticsView(nullptr);
		m_pCore->GetUIMgr()->GetTabWidget()->addTab(pView, "test");
	}
