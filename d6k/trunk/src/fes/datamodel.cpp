...
{
	return static_cast<int> (m_nColumnCount);
}

int CDataModel::rowCount(const QModelIndex &parent /*= QModelIndex()*/) const
{
	return static_cast<int>(m_nRowCount);
}
...
float CDataModel::GetVal(IO_VARIANT& val, int32u nIddType)const
{
	switch (nIddType)
	{
		case DT_BOOLEAN:
...
QVariant CDataModel::ShowUserVarData(int nRow, int nColumn) const
{
...
QVariant CDataModel::ShowSysVarData(int nRow, int nColumn) const
{
...
bool CDataModel::GetTotalInfo()
{
	switch (m_nDataType)
	{
		case PROJECT_TYPE:
		{
			for (int32u i = 0; i < m_pMem->GetChannelCount(); ++i)
			{
				CHANNEL *pChannel;
		...
		case USERVAR_TYPE:
		{
			size_t nCount = m_pMem->GetUserVarCount();
			for (int32u i = 0; i < nCount; ++i)
			{
				VARDATA * pVardata = m_pMem->GetUserVarByIndex(i);
				Q_ASSERT(pVardata);
				m_arrUserVars.push_back(pVardata);
			}
			break;
		}
		case SYSVAR_TYPE:
		{
			size_t nCount = m_pMem->GetSysVarCount();
			for (int32u i = 0; i < nCount; ++i)
			{
				VARDATA * pVardata = m_pMem->GetSysVarByIndex(i);
				Q_ASSERT(pVardata);
				m_arrSysVars.push_back(pVardata);
			}
			break;
		}
		default:
