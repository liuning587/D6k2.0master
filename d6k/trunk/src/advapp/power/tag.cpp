...
CDinData::CDinData(std::string strTagName) : CVar(strTagName)
{
...
bool CDinData::GetOccNoByTagName()
{
	...
	Q_ASSERT(m_strTagName.empty() == false);
	if (m_strTagName.empty() == true)
	{// TODO:  LOG
	...
	bRet = ::GetOccNoByTagName(m_strTagName.c_str(), &m_pOccNoItem->NodeOccNo, &m_pOccNoItem->IddType, &m_pOccNoItem->TagOccNo, &m_pOccNoItem->FiledID);
	if (bRet == false)
	{
		szLog = QString(QObject::tr("[%1] convert OccNo failed.")).arg(m_strTagName.c_str());
		LogMsg(szLog.toStdString().c_str(), 0);
	}
...
bool CDinData::GetRealTimeData()
{
	...
	{// TODO: log
		QString szLog;
		szLog = QString(QObject::tr("[%1] GetRTData failed.")).arg(m_strTagName.c_str());
		LogMsg(szLog.toStdString().c_str(), 0);
	}
...
CAinData::CAinData(std::string strTagName) : CVar(strTagName)
{
